...
export function exportCSV(whiskies) {
  const headers = ['Name','Distillery','Region / Origin','Style','Age / Maturation',
  ...
  const rows = whiskies.map(w =>
    [w.name, w.distillery, w.origin, w.type, w.age, w.price, w.date,
  ...
  a.href = url
  a.download = 'dram-journal-' + new Date().toISOString().slice(0, 10) + '.csv'
  a.click()
