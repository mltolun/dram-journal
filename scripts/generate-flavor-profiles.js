 ...
 *
 * Rate limits: 1 RPM, 15K TPM, 14.4K RPD
 * Strategy: process up to BATCH_LIMIT whiskies per run, sleep 2.5s between calls (30 RPM limit).
 * Run manually via GitHub Actions workflow_dispatch — each run picks up
 * where the last one left off using the flavor_status column.
 *
 * Usage (local):
 ...
 *   SLEEP_MS     — ms to wait between calls (default: 2500 = ~24 RPM, safely under 30)
...
const SLEEP_MS             = parseInt(process.env.SLEEP_MS     || '2500')  // 30 RPM = 2s between calls + buffer
  ...
  for (let i = 0; i < whiskies.length; i++) {
    ...
    try {
      ...
      await sb.from('catalogue')
        .update({ flavor_status: 'error' })
        .eq('id', whisky.id)

      failed++
    }

    // Sleep between calls to respect 1 RPM — skip after last item
